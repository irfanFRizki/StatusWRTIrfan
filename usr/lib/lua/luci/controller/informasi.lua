module("luci.controller.informasi", package.seeall)
 ...
 -----------------------------------------------------------------------
 -- fungsi untuk mengarahkan pengguna ke captive portal setelah di-kick
function action_kick()
    local ip = luci.http.formvalue("ip")
    -- 1. Blokir IP menggunakan firewall
    os.execute("iptables -I FORWARD -s " .. ip .. " -j DROP")
    
    -- 2. Arahkan ke captive portal dengan teknik DNS Redirection
    os.execute("iptables -t nat -I PREROUTING -s " .. ip .. " -p tcp --dport 80 -j DNAT --to-destination " .. luci.http.getenv("SERVER_ADDR") .. ":80")
    os.execute("iptables -t nat -I PREROUTING -s " .. ip .. " -p tcp --dport 443 -j DNAT --to-destination " .. luci.http.getenv("SERVER_ADDR") .. ":80")
    
    -- 3. Redirect ke halaman captive portal
    luci.http.redirect(luci.dispatcher.build_url("informasi/captive") .. "?ip=" .. ip)
end

 -----------------------------------------------------------------------
 -- fungsi untuk menangani request akses
function action_request_access()
    local ip = luci.http.formvalue("ip")
    local reason = luci.http.formvalue("reason")
    
    -- 1. Simpan request ke file
    os.execute('echo "' .. ip .. '|' .. reason .. '" >> /tmp/access_requests.txt')
    
    -- 2. Kirim notifikasi ke Telegram dengan tombol unblock
    local telegram_msg = string.format(
        '📡 <b>Permintaan Akses Baru</b>\\nIP: <code>%s</code>\\nAlasan: %s',
        ip,
        reason:gsub('"', '\\"')
    )
    local telegram_cmd = string.format(
        'python3 /usr/bin/send_telegram.py "%s" "%s"',
        telegram_msg,
        ip
    )
    os.execute(telegram_cmd)
    
    -- 3. Beri respon ke user
    luci.template.render("informasi/request_received", {ip = ip})
end

 -----------------------------------------------------------------------
 -- Endpoint untuk Unblock dari Telegram
function action_telegram_callback()
    local callback_data = luci.http.formvalue("callback_data")
    
    if callback_data:startswith("unblock_") then
        local ip = callback_data:sub(9)  -- Ambil IP dari callback_data
        action_unblock(ip)
        luci.http.prepare_content("application/json")
...
-- Pindahkan logika unblock ke fungsi terpisah
function action_unblock(ip)
    os.execute("iptables -D FORWARD -s " .. ip .. " -j DROP")
    os.execute("iptables -t nat -D PREROUTING -s " .. ip .. " -p tcp --dport 80 -j DNAT --to-destination " .. luci.http.getenv("SERVER_ADDR") .. ":80")
    os.execute("iptables -t nat -D PREROUTING -s " .. ip .. " -p tcp --dport 443 -j DNAT --to-destination " .. luci.http.getenv("SERVER_ADDR") .. ":80")
end
 end
